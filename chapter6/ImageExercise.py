...
class Image:
    ...
    def resize(self, width=None, height=None):
        """Resizes to the given dimensions; return True if chages made"""
        ...
        if width >= self.__width and height >= self.__height:
            # if both greater, no delete, return True
            self.__width = width
            self.__height = height
            return True
        self.__width = width
        self.__height = height
        for x, y in list(self.__data.keys()):  # (x,y)
            if x >= self.__width or y >= self.__height:
                del self.__data[(x, y)]
        self.__colors = set(self.__data.values()) | {self.__background}  # refresh the color list
        return True
